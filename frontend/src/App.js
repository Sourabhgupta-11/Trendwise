...
const PageLoader = () => (
  <div style={{
    ...
    <div style={{display:"flex",gap:8}}>
      {[0,1,2].map(i=>(
        <span key={i} style={{
          width:10,height:10,borderRadius:"50%",background:"#818cf8",display:"inline-block",
          animation:"tw-bounce 1.2s ease-in-out infinite",animationDelay:`${i*0.2}s`,
        }}/>
      ))}
    </div>
...
const AppWrapper = () => {
  const [user,       setUser]       = useState(() => { try { return JSON.parse(sessionStorage.getItem("tw_user")); } catch { return null; } });
  ...
  return (
    <>
      {loggingOut && (
        <div style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(15,23,42,.85)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <PageLoader />
        </div>
      )}
